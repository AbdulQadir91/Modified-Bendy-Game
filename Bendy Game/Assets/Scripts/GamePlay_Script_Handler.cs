...
public class GamePlay_Script_Handler : MonoBehaviour
{
	...
	public GameObject miniMap,caughtByBendyImage,env;
	//---------------------------- Other Stuff END ------------------------//
	...
	void Awake()
	{
		...
		if (SoundManager_Handler.soundManagerInstance != null) {
			...
			SoundManager_Handler.soundManagerInstance.changeMusic_Volume (0.2f);
		}
	...
	public void caughtByBendy()
	{
		StartCoroutine (gotCaughtByBendy());
	}

	IEnumerator gotCaughtByBendy()
	{
	...
	public void levelEndEvent(bool temp)
	{
		if (temp) {
			StartCoroutine (delayedCallComplete());
		} 
		else {
			StartCoroutine (delayedCallFail());	
		}
	}

	IEnumerator delayedCallComplete()
	{
		yield return new WaitForSeconds (3f);
		levelComplete_EventCall ();
	}

	IEnumerator delayedCallFail()
	{
		yield return new WaitForSeconds (3f);
		levelComplete_EventCall ();
	}

	void levelFail_EventCall()
	{
